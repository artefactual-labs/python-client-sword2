...
class TestConnection(TestController):
    ...
    def test_04_init_from_sss_then_get_doc(self):
        conn = Connection(
            "http://localhost:%s/sd-uri" % PORT_NUMBER,
            user_name="sword",
            user_pass="sword",
        )
    ...
    def test_11_Multipart_POST_then_update_metadata_on_Edit_IRI(self):
        ...
        dr = conn.update_metadata_for_resource(
            edit_iri=deposit_receipt.edit, metadata_entry=e
        )
    ...
    def test_15_Metadata_POST_to_sss_w_coliri(self):
        ...
        dr = conn.create(
            metadata_entry=e, col_iri=conn.sd.workspaces[0][1][0].href, in_progress=True
        )
        assert dr.code == 201
