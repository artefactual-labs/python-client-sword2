...
class ServiceDocument:
    ...
    def load_document(self, xml_response):
        try:
            ...
            sd_l.info(
                "Initial SWORD2 validation checks on service document - Valid document? %s"
                % self.valid
            )
            ...
            # exceptions...
            sd_l.error(
                "Could not parse the Service Document response from the server - %s" % e
            )
            ...
            for workspace in workspaces:
                ...
                for col in cols:
                    ...
                    for accept in accepts:
                        ...
                        if multipart is not None:
                            if (
                                ...
                                and multipart != "multipart/related"
                            ):
                                multipart_accept_valid = False
                                sd_l.debug(
                                    "Multipart accept alternate is incorrect: "
                                    + str(multipart)
                                )
                        else:
                    ...
                    if not multipart_accept_valid or not accept_valid:
                        sd_l.debug(
                            "Either the multipart accept or the accept fields were invalid (see above debug)"
                        )
                        valid = False
