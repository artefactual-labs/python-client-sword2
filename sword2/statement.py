...
class Atom_Statement_Entry(Deposit_Receipt, Statement_Resource):
    ...
    def _parse_depositors(self):
        ...
        if do is not None and do.text is not None and do.text.strip() != "":
            try:
                self.deposited_on = datetime.strptime(
                    do.text.strip(), "%Y-%m-%dT%H:%M:%SZ"
                )  # e.g. 2011-03-02T20:50:06Z
            except Exception as e:
...
class Atom_Sword_Statement(Sword_Statement):
    def __init__(self, xml_document=None):
        ...
        if self.valid:
            ...
            s_l.warn(
                "Statement did not parse as valid, so the content will"
...
class Ore_Statement_Resource(Statement_Resource):
    ...
    def __str__(self):
        # FIXME: unfinished ...
        return (
            "URI: %s ; is_original_deposit: %s ; packaging_uris: %s ; deposited_on: %s"
        )
...
class Ore_Sword_Statement(Sword_Statement):
    ...
    def _enumerate_descriptions(self):
        ...
        s_l.debug(
            "First pass on ORE statement yielded the following Aggregated Resources: "
            + str(aggregated_resource_uris)
        )
        s_l.debug(
            "First pass on ORE statement yielded the following Original Deposits: "
            + str(original_deposit_uris)
        )
        s_l.debug(
            "First pass on ORE statement yielded the following States: "
            + str(state_uris)
        )
        ...
        for desc in self.dom.findall(NS["rdf"] % "Description"):
            ...
            if about in state_uris:
                ...
                if (
                    sdesc is not None
                    and sdesc.text is not None
                    and sdesc.text.strip() != ""
                ):
                ...
                s_l.debug(
                    "Is Aggregated Resource an original deposit? "
                    + str(is_original_deposit)
                )
                ...
                if do is not None and do.text is not None and do.text.strip() != "":
                    try:
                        deposited_on = datetime.strptime(
                            do.text.strip(), "%Y-%m-%dT%H:%M:%SZ"
                        )  # e.g. 2011-03-02T20:50:06Z
                ...
                if dobo is not None and dobo.text is not None and db.text.strip() != "":
                    ...
                    s_l.debug(
                        "Registering Deposited On Behalf Of: " + deposited_on_behalf_of
                    )

                ose = Ore_Statement_Resource(
                    about,
                    is_original_deposit,
                    packaging_uris,
                    deposited_on,
        ...
        s_l.debug(
            "Undescribed Aggregated Resource URIs: " + str(aggregated_resource_uris)
        )
    ...
    def _validate(self):
        ...
        if self.dom.tag.lower() != NS["rdf"] % "rdf" and self.dom.tag.lower() != "rdf":
            s_l.info(
                "Validation of Ore Statement failed, as root tag is not RDF: "
                + self.dom.tag
            )
            valid = False
        ...
        if describes_uri != aggregation_uri:
            s_l.info(
                "Validation of Ore Statement failed; ore:describes URI does not match Aggregation URI: "
                + describes_uri
                + " != "
                + aggregation_uri
            )
        ...
        if rem_uri not in is_described_by_uris:
            s_l.info(
                "Validation of Ore Statement failed; Resource Map URI does not match one of ore:isDescribedBy URIs: "
                + rem_uri
                + " not in "
                + str(is_described_by_uris)
            )
            valid = False
