...
class Deposit_Receipt:
    def __init__(
        ...
        if self.dom is not None:
            ...
            d_l.info(
                "Initial SWORD2 validation checks on deposit receipt - Valid document? %s"
                % self.valid
            )
    ...
    def handle_link(self, e):
        ...
        if rel:
            if rel == "edit":
                ...
                # there is no 'type'
                if (
                    self._normalise_mime(e.attrib.get("type"))
