...
from sword2 import Error_Document
from sword2 import Ore_Sword_Statement
from sword2 import UrlLib2Layer
...
class TestConnection(TestController):
    ...
    def test_13_advanced_retrieve_content_em_iri(self):
        ...
        resource = conn.get_resource(
            content_iri=receipt.edit_media, packaging=packaging, on_behalf_of=SSS_OBO
        )
    ...
    def test_23_basic_add_content_to_resource_single_file(self):
        ...
        with open(PACKAGE) as pkg:
            new_receipt = conn.add_file_to_resource(
                receipt.edit_media, pkg, "addition.zip", mimetype=PACKAGE_MIME
            )
    ...
    def test_37_error_target_owner_unknown(self):
        ...
        with open(PACKAGE) as pkg:
            receipt = conn.create(
                ...
                in_progress=True,
                suggested_identifier="zyxwvutsrq",
                on_behalf_of="richard",
            )  # we expressly set the wrong obo on the request rather than the connection

        assert receipt.code == 403
        assert isinstance(receipt, Error_Document)
        assert (
            receipt.error_href == "http://purl.org/net/sword/error/TargetOwnerUnknown"
        )
    ...
    def test_41_get_service_document_unauthorised(self):
        conn = Connection(
            SSS_URL, user_name="alsdkfjsdz", user_pass="ZAKJKLASJDF", http_impl=http
        )
        conn.get_service_document()
