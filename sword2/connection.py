...
class Connection:
    ...
    def __init__(
        ...
        if keep_history:
            conn_l.info(
                "keep_history=True--> This instance will keep a JSON-compatible transaction log of all (SWORD/APP) activities in 'self.history'"
            )
        ...
        if self.sd_iri and download_service_document:
            ...
            conn_l.debug(
                "Getting service document and dealing with the response: %s s"
                % self._t.time_since_start("get_service_document")[1]
            )
    ...
    def _return_error_or_exception(self, cls, resp, content):
        ...
        if self.raise_except:
            ...
            if resp.get("content-type", "").startswith("text/xml") or resp.get(
                "content-type", ""
            ).startswith("application/xml"):
                conn_l.info(
                    "Returning an error document, due to HTTP response code %s"
                    % resp.status
                )
    ...
    def _handle_error_response(self, resp, content):
        ...
        """
        conn_l.debug(f"Error body received from server: {str(content)}")

        if resp["status"] == 401:
            conn_l.error(
                "You are unauthorised (401) to access this document on the server. Check your username/password credentials and your 'On Behalf Of'"
            )
            return self._return_error_or_exception(NotAuthorised, resp, content)
        elif resp["status"] == 403:
            conn_l.error(
                "You are Forbidden (403) to POST to '%s'. Check your username/password credentials and your 'On Behalf Of'"
            )
            ...
            conn_l.error(
                "Server error occured. Response headers from the server:\n%s" % resp
            )
            return self._return_error_or_exception(ServerError, resp, content)
        else:
            conn_l.error(
                "Unknown error occured. Response headers from the server:\n{}\n{}".format(
                    resp, content
                )
            )
    ...
    def _cache_deposit_receipt(self, d):
        ...
        if self.keep_cache:
            ...
            conn_l.debug(f"Caching document (Edit-IRI:{d.edit}) - at {timestamp}")
            ...
            conn_l.debug(
                "Caching request denied - deposit receipt caching is set to 'False'"
            )
    ...
    def get_service_document(self):
        ...
        if resp["status"] == 200:
            ...
            conn_l.error(
                "You are unauthorised (401) to access this document on the server. Check your username/password credentials"
            )
        else:
    ...
    def _make_request(
        ...
        if empty:
            ...
            resp, content = self.h.request(
                target_iri, method, headers=headers, payload=data
            )
            _, took_time = self._t.time_since_start(request_type)
            if self.history:
                self.history.log(
                    request_type + ": Metadata-only resource request",
                    sd_iri=self.sd_iri,
                    target_iri=target_iri,
                    method=method,
                    response=resp,
            ...
            multicontent_type, payload_data = create_multipart_related(
                [
                    {
                        "key": "atom",
            ...
            headers["Content-Type"] = (
                multicontent_type + '; type="application/atom+xml"'
            )
            headers["Content-Length"] = str(
                len(payload_data)
            )  # must be str, not int type
            resp, content = self.h.request(
                target_iri, method, headers=headers, payload=payload_data
            )
            _, took_time = self._t.time_since_start(request_type)
            if self.history:
                self.history.log(
                    request_type + ": Multipart resource request",
                    sd_iri=self.sd_iri,
                    target_iri=target_iri,
                    response=resp,
                    headers=headers,
            ...
            resp, content = self.h.request(
                target_iri, method, headers=headers, payload=payload
            )
            _, took_time = self._t.time_since_start(request_type)
            if self.history:
                self.history.log(
                    request_type + ": simple resource request",
                    sd_iri=self.sd_iri,
                    target_iri=target_iri,
                    method=method,
                    response=resp,
            ...
            conn_l.error(
                "Parameters were not complete: requires a metadata_entry, or a payload/filename/packaging or both"
            )
            raise Exception(
                "Parameters were not complete: requires a metadata_entry, or a payload/filename/packaging or both"
            )
        ...
        if resp["status"] == 201:
            ...
            if len(content) > 0:
                ...
                if d.parsed:
                    conn_l.info(
                        "Server response included a Deposit Receipt. Caching a copy in .resources['%s']"
                        % d.edit
                    )
            ...
            return Deposit_Receipt(
                response_headers=dict(resp), location=location, code=204
            )
            ...
            if (
                self._normalise_mime(content_type).startswith(
                ...
                if w == workspace:
                    for c in collections:
                        if c.title == collection:
                            conn_l.debug(
                                "Matched: Workspace='{}', Collection='{}' ==> Col-IRI='{}'".format(
                                    workspace, collection, c.href
                                )
                            )
                            col_iri = c.href
                            break
    ...
    def update(
        self,
        metadata_entry=None,  # required for a metadata update
    ...
    def add_file_to_resource(
        ...
        """
        conn_l.info(
            "Appending file to a deposit via Edit-Media-IRI %s" % edit_media_iri
        )
        return self._make_request(
            target_iri=edit_media_iri,
            payload=payload,
    ...
    def append(
        ...
        if not se_iri:
            if dr is not None:
                ...
                if se_iri:
                    ...
                    if se_iri:
                        conn_l.info(
                            "Complete deposit using the Edit-IRI %s as SWORD2-Edit-IRI not available"
                            % se_iri
                        )
                    else:
                        raise Exception(
                            "No SWORD2-Edit-IRI was given and no suitable IRI was found in the deposit receipt."
                        )
            else:
        ...
        conn_l.info(
            "Adding new file, metadata or both to a SWORD deposit via SWORD-Edit-IRI %s"
            % se_iri
        )
        return self._make_request(
            target_iri=se_iri,
    ...
    def delete(self, resource_iri, on_behalf_of=None):
        """
        Delete resource
    ...
    def delete_content_of_resource(
        self, edit_media_iri=None, on_behalf_of=None, dr=None
    ):
        """
        ...
        if not edit_media_iri:
            if dr is not None:
                ...
                if edit_media_iri:
                    conn_l.info(
                        "Deleting Resource via Edit-Media-IRI %s" % edit_media_iri
                    )
                else:
                    raise Exception(
                        "No Edit-Media-IRI was given and no suitable IRI was found in the deposit receipt."
                    )
            else:
        ...
        return self.delete(edit_media_iri, on_behalf_of=on_behalf_of)

    def delete_container(self, edit_iri=None, on_behalf_of=None, dr=None):
        """
        ...
        if not edit_iri:
            if dr is not None:
                ...
                if edit_iri:
                    ...
                    raise Exception(
                        "No Edit-IRI was given and no suitable IRI was found in the deposit receipt."
                    )
            else:
        ...
        return self.delete(edit_iri, on_behalf_of=on_behalf_of)

    def complete_deposit(self, se_iri=None, on_behalf_of=None, dr=None):
        """
        ...
        if not se_iri:
            if dr is not None:
                ...
                if se_iri:
                    conn_l.info(
                        "Complete deposit using the SWORD2-Edit-IRI %s" % se_iri
                    )
                else:
    ...
    def update_files_for_resource(
        ...
        if not edit_media_iri:
            if dr is not None:
                ...
                if edit_media_iri:
                    conn_l.info(
                        "Update Resource via Edit-Media-IRI %s" % edit_media_iri
                    )
                else:
                    raise Exception(
                        "No Edit-Media-IRI was given and no suitable IRI was found in the deposit receipt."
    ...
    def update_metadata_for_resource(
        self,
    ...
    def update_metadata_and_files_for_resource(
        self,
        metadata_entry,  # required
        payload,  # These need to be set to upload a file
        filename,  # According to spec, "The client MUST supply a Content-Disposition header with a filename parameter
    ...
    def get_deposit_receipt(self, edit_iri):
        ...
        if response.code == 200:
            ...
            if d.parsed:
                conn_l.info(
                    "Server responsed with a Deposit Receipt. Caching a copy in .resources['%s']"
                    % d.edit
                )
    ...
    def get_ore_sword_statement(self, sword_statement_iri):
        ...
        conn_l.debug(
            "Trying to GET the ORE Sword Statement at %s." % sword_statement_iri
        )
        response = self.get_resource(
            sword_statement_iri, headers={"Accept": "application/rdf+xml"}
        )
        if response.code == 200:
            # try:
            if True:
                conn_l.debug(
                    "Attempting to parse the response as a ORE Sword Statement"
                )
    ...
    def get_atom_sword_statement(self, sword_statement_iri):
        ...
        # get the statement first
        conn_l.debug(
            "Trying to GET the ATOM Sword Statement at %s." % sword_statement_iri
        )
        response = self.get_resource(
            sword_statement_iri, headers={"Accept": "application/atom+xml;type=feed"}
        )
        if response.code == 200:
            # try:
            if True:
                conn_l.debug(
                    "Attempting to parse the response as a ATOM Sword Statement"
                )
    ...
    def get_resource(
        self, content_iri=None, packaging=None, on_behalf_of=None, headers={}, dr=None
    ):
        """
        ...
        if not content_iri:
            if dr is not None:
                ...
                if content_iri:
                    ...
                    raise Exception(
                        "No Content-IRI was given and no suitable IRI was found in the deposit receipt."
                    )
            else:
        ...
        if self.honour_receipts and packaging:
            ...
            conn_l.debug(
                "Checking that the packaging format '%s' is available." % content_iri
            )
            conn_l.debug("Cached Cont-IRI Receipts: %s" % list(self.cont_iris.keys()))
            if content_iri in list(self.cont_iris.keys()):
                if packaging not in self.cont_iris[content_iri].packaging:
                    conn_l.error(
                        "Desired packaging format '%' not available from the server, according to the deposit receipt. Change the client parameter 'honour_receipts' to False to avoid this check."
                    )
                    return self._return_error_or_exception(
                        PackagingFormatNotAvailable, {}, ""
                    )
        if on_behalf_of:
            headers["On-Behalf-Of"] = on_behalf_of
        elif self.on_behalf_of:
        ...
        if packaging:
            conn_l.info(
                "IRI GET resource '{}' with Accept-Packaging:{}".format(
                    content_iri, packaging
                )
            )
        else:
        ...
        if resp["status"] == 200:
            conn_l.debug(
                "Cont_IRI GET resource successful - got {} bytes from {}".format(
                    len(content), content_iri
                )
            )
    ...
    def delete_file(self, file_edit_media, on_behalf_of=None):
        ...
        """
        return self._make_request(
            file_edit_media, on_behalf_of=on_behalf_of, method="DELETE"
        )
